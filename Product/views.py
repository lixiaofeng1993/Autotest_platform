...
class Project:
    ...
    def create(request):
        ...
        p.team_id = request.session.get('tid', None)
        try:
            p.clean()
    ...
    def delete(request, project_id):
        ...
        loginConfigs = LoginConfig.objects.filter(project_id=p.id)
        if loginConfigs:
    ...
    def find(request):
        ...
        try:
            projects = project.objects.filter(createTime__lt=end_time, createTime__gt=start_time, team_id=tid,
...
class Environment:
    ...
    def create(request):
        ...
        try:
            ...
            return JsonResponse.BadRequest('json格式错误')
        e = environment()
        e.project_id = parameter.get('projectId', None)
        ...
        e.team_id = request.session.get('tid', None)
        try:
            e.clean()
        except ValidationError as ve:
            return JsonResponse.BadRequest(','.join(ve.messages))
    ...
    def edit(request, environment_id):
        ...
        # 判断重复
        e.name = e.name.strip()
        if environment.objects.filter(name__exact=e.name, project_id=e.project_id).exclude(id=e.id):
    ...
    def get(request, environment_id):
        ...
        result = model_to_dict(e, ['id', 'project_id', 'name', 'host', 'remark'])
        return JsonResponse.OK(message='ok', data=result)
...
class Page:
    ...
    def create(request):
        ...
        p.name = parameter.get('name', '')
        p.remark = parameter.get('remark', '')
        p.project_id = parameter.get('projectId', None)
        p.team_id = request.session.get('tid', None)
        try:
            p.clean()
        except ValidationError as ve:
    ...
    def find(request):
        ...
        tid = request.session.get('tid', None)
        try:
            pages = page.objects.filter(createTime__lt=end_time, createTime__gt=start_time, team_id=tid,
...
class Element:
    ...
    def find(request):
        ...
        tid = request.session.get('tid', None)
        try:
            elements = element.objects.filter(createTime__lt=end_time, createTime__gt=start_time, team_id=tid,
                                              name__contains=name).order_by('-createTime')
            if project_id:
...
class Keyword:
    ...
    def create(request):
        ...
        kw.project_id = parameter.get('projectId', None)
        kw.project_id = int(kw.project_id) if str(kw.project_id).isdigit() and str(kw.project_id) != '0' else None
        kw.team_id = request.session.get('tid', None)
        try:
            kw.clean()
        except ValidationError as ve:
            return JsonResponse.BadRequest(','.join(ve.messages))
        kw.name = kw.name.strip()
        project_ids = [0, kw.project_id]
        ks = get_model(keyword, False, name__exact=kw.name, project_id__in=project_ids)
        if ks:
    ...
    def edit(request, keyword_id):
        ...
        kw.name = kw.name.strip()
        project_ids = [0, kw.project_id]
        if keyword.objects.filter(name__exact=kw.name, project_id__in=project_ids).exclude(id=kw.id):
            return JsonResponse.BadRequest('已存在该关键字,请修改后重试')
        # 保存
        if kw.type == 2:
            from Product.models import Params
    ...
    def find(request):
        ...
        try:
            ks = keyword.objects.filter(createTime__lt=end_time, createTime__gt=start_time,
                                        name__contains=name).order_by('-createTime')
            if project_id:
                ks = ks.filter(project_id__in=[0, project_id])
            if t:
                ks = ks.filter(type=t)
            total = len(ks)
        except:
...
class TestCase:
    ...
    def create(request):
        ...
        tc.beforeLogin = list()
        tc.team_id = request.session.get('tid', None)
    ...
    def edit(request, testcase_id):
        ...
        # 判断参数有效性
        tc.project_id = parameter.get('projectId', 0)
        tc.project_id = int(tc.project_id) if str(tc.project_id).isdigit() else 0
        tc.level = parameter.get('level', 1)
        tc.level = int(tc.level) if str(tc.level).isdigit() else 0
        tc.title = parameter.get('title', None)
        tc.remark = parameter.get('remark', None)
    ...
    def find(request):
        ...
        tid = request.session.get('tid', None)
        try:
            tcs = testcase.objects.filter(createTime__lt=end_time, createTime__gt=start_time, team_id=tid,
    ...
    def test(request, testcase_id):
        ...
        r = Result.objects.create(project_id=tc.project_id, testcase_id=tc.id, checkValue=tc.checkValue,
                                  ...
                                  steps=tc.steps, parameter=tc.parameter, team_id=tid,
...
class TestResult:
    ...
    def find(request):
        ...
        testcase_id = int(testcase_id) if str(testcase_id).isdigit() and int(testcase_id) >= 1 else 0
        title = parameter.get('title') if parameter.get('title', '') else ''
        title = title.strip()
        status = parameter.get('status') if parameter.get('status', 0) else 0
        status = int(status) if str(status).isdigit() and int(status) >= 1 else 0
        ...
        tcs_list = list()
        for re in res:
            dic = model_to_dict(re, ['id', 'project', 'task', 'testcase', 'title', 'status', 'checkType',
                                     'checkValue', 'checkText', 'selectText'])
            dic['parameter'] = json.loads(str(re.parameter)) if re.parameter else []
            task = get_model(Task, id=re.task_id)
            dic['taskName'] = task.name if task else ''
            tc = get_model(project, id=re.project_id)
    ...
    def delete(request, result_id):
        ...
        if tr:
            ...
            sts = get_model(SplitResult, get=False, result_id=result_id)
    ...
    def get(request, result_id):
        ...
        for index, _id in enumerate(json.loads(result_id_list)):
            ...
            result = model_to_dict(re, ['id', 'project', 'task', 'testcase', 'title', 'status', 'checkType',
                                        'checkValue', 'checkText', 'selectText', 'steps'])
            if status and status != str(result['status']):
                continue
            tc = testcase.objects.get(id=result['testcase'])
            ...
            split = get_model(SplitResult, False, result_id=re.id)
            splitResult = list()
            step_num = 0
            error_name = ''
            for s in split:
                ...
                sd['browser'] = get_model(Browser, id=s.browser_id).name if get_model(Browser, id=s.browser_id) else ''
                sd['environment'] = get_model(environment, id=s.environment_id).name if get_model(environment,
                                                                                                  id=s.environment_id) else ''
...
class Public:
    ...
    def data(request):
        ...
        try:
            ...
            return JsonResponse.BadRequest('json格式错误')
        if parameter:
            en = get_model(environment, get=False, project_id=parameter.get('projectId', 0))
            if len(en) == 1:
                for e in en:
...
class TestTasks:
    ...
    def create(request):
        ...
        kwargs.update(
            {'testcases': testcases, 'browsers': browsers, 'name': name, 'tid': request.session.get('tid', None)})
        t.name = name
    ...
    def test(request, task_id):
        ...
        for tc in testcases:
            ...
            tid = request.session.get('tid', None)
            r = Result.objects.create(project_id=tc.project_id, testcase_id=tc.id, checkValue=tc.checkValue,
                                      checkText=tc.checkText, selectText=tc.selectText, team_id=tid,
                                      checkType=tc.checkType, title=t.name, beforeLogin=tc.beforeLogin,
                                      steps=tc.steps, parameter=tc.parameter,
                                      browsers=json.dumps(browsers, ensure_ascii=False),
                                      environments=json.dumps(environments, ensure_ascii=False), task_id=t.id)
...
class Login:
    ...
    def create(request):
        try:
            ...
            return JsonResponse.BadRequest('json格式错误')
        l = LoginConfig()
        l.project_id = parameter.get('projectId', 0)
        l.project_id = int(l.project_id) if str(l.project_id).isdigit() else 0
        ...
        l.team_id = request.session.get('tid', None)
        try:
            l.clean()
        except ValidationError as ve:
            return JsonResponse.BadRequest(','.join(ve.messages))
        ...
        l.name = l.name.strip()
        lcs = get_model(LoginConfig, False, name__exact=l.name, project_id__in=[0, l.project_id])
        if lcs:
    ...
    def delete(request, login_id):
        ...
        try:
            EnvironmentLogin.objects.filter(login_id=l.id).delete()
            l.delete()
        except:
            return JsonResponse(500, '服务器发生错误')
        return JsonResponse.OK()
    ...
    def edit(request, login_id):
        ...
        l.steps = json.dumps(l.steps, ensure_ascii=False)
        l.name = l.name.strip()
        if LoginConfig.objects.filter(name__exact=l.name, project_id__in=[0, l.project_id]).exclude(id=l.id):
    ...
    def find(request):
        ...
        tid = request.session.get('tid', None)
        try:
            ls = LoginConfig.objects.filter(createTime__lt=end_time, createTime__gt=start_time, team_id=tid,
                                            name__contains=name).order_by('-createTime')
            if project_id != 0:
                ls = ls.filter(project_id__in=[0, project_id])
            total = len(ls)
        except:
            return JsonResponse(400, '时间参数错误')
        ...
        ls_list = list()
        for l in ls:
            dic = model_to_dict(l, ['id', 'project', 'name', 'remark', 'checkType', 'checkValue', 'checkText',
                                    'selectText'])
            dic['params'] = json.loads(l.params) if l.params else None
            dic['steps'] = json.loads(l.steps) if l.steps else None
            dic['projectName'] = '' if l.project_id == 0 else get_model(project, id=l.project_id).name
            dic['bindNum'] = len(EnvironmentLogin.objects.filter(login_id=l.id))
    ...
    def get(request, login_id):
        ...
        if not l:
            return JsonResponse.BadRequest('该测试用例不存在')
        dic = model_to_dict(l,
                            ['id', 'project', 'name', 'remark', 'checkType', 'checkValue', 'checkText', 'selectText'])
        dic['projectName'] = '' if l.project_id == 0 else get_model(project, id=l.project_id).name
        dic['params'] = json.loads(l.params) if l.params else None
        ...
        dic['steps'] = steps_
        dic['createTime'] = l.createTime.strftime('%Y-%m-%d %H:%M:%S')
        els = EnvironmentLogin.objects.filter(login_id=l.id)
        e_list = []
        for el in els:
            ep = model_to_dict(el, ['id', 'environment'])
            environment_ = get_model(environment, id=el.environment_id)
    ...
    def bind(request, login_id):
        ...
        try:
            ...
            return JsonResponse.BadRequest('json格式错误')
        environment_id = parameter.get('environmentId', 0)
        if not (str(environment_id).isdigit() and int(environment_id) > 0):
            return JsonResponse.BadRequest('请选择正确的环境')
        parameter = parameter.get('parameter', {})
        if EnvironmentLogin.objects.filter(environment_id=environment_id, login_id=lc.id):
            return JsonResponse(400, '该登陆配置与该环境已绑定')
        el = EnvironmentLogin()
        el.login_id = lc.id
        el.environment_id = environment_id
    ...
    def copy(request, login_id):
        ...
        result = model_to_dict(lc,
                               ['id', 'project', 'name', 'remark', 'checkType', 'checkValue', 'checkText',
                                'selectText'])
        result['projectName'] = get_model(project, id=lc.project_id).name
