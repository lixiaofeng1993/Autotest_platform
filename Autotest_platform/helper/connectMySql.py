...
from Autotest_platform.helper import readConfig
import logging
...
class SqL:
        ...
        if job:
            ...
            try:
                self.conn = pymysql.connect(host=readConfig.MySQL_host, database=readConfig.MySQL_database,
                                            ...
                                            use_unicode=True, charset='utf8')
                log.info('dbwaxiansheng 数据库连接成功')
    ...
    def execute_sql(self, sql, dict_type=False, num=1):
        ...
        try:
            with cur as cur:
                cur.execute(sql)  # 执行sql
            if 'delete' in sql or 'insert' in sql or 'update' in sql or 'DELETE' in sql:
                self.conn.commit()  # 提交
            else:
                if num == 1:  # 返回一条数据
                    data = cur.fetchone()
...
if __name__ == '__main__':
    r = SqL(job=False)
    data = r.execute_sql(
        'select * from browser;', dict_type=True)
    print(data)
